...
class AIMentorScreenState extends State<AIMentorScreen> {
  ...
  // Загружаем историю чата напрямую из SharedPreferences
  Future<void> _loadChatHistory() async {
    try {
      ...
      if (userId != null) {
        ...
        if (jsonString != null) {
          try {
            ...
            setState(() {
              _messages.clear();
              // Конвертируем историю в формат UI
            ...
            developer.log('Error parsing chat history: $e', name: 'AIMentorScreen');
            setState(() {
              _isLoadingHistory = false;
            });
          }
      ...
      developer.log('Error loading chat history: $e', name: 'AIMentorScreen');
      setState(() {
        _isLoadingHistory = false;
      });
    }
  ...
  void _sendMessage() async {
    ...
    try {
      ...
      if (_requestType == 'goal') {
        response = await _aiService.generateGoal(
            message, authProvider.token ?? '', userId!);
      } else {
        response = await _aiService.generateEvent(
            message, authProvider.token ?? '', userId!);
      }
